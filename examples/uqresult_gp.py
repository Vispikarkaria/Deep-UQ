...
def main() -> None:
    ...
    print("exact_gp:", exact_uq.mean.shape, exact_uq.total_var.shape if exact_uq.total_var is not None else None)

    sparse = SparseGaussianProcessRegressor(num_inducing=24, num_iterations=200)
    sparse.fit(x, y)
    sparse_uq = sparse.predict_uq(x_star)
    print("sparse_gp:", sparse_uq.mean.shape, sparse_uq.total_var.shape if sparse_uq.total_var is not None else None)
