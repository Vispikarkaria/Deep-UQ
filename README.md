...
| Gaussian Processes | **1)** `GaussianProcessRegressor`<br>**2)** `SparseGaussianProcessRegressor`<br>**3)** `GaussianProcessClassifier`<br>**4)** `OneVsRestGaussianProcessClassifier`<br>**5)** `HeteroscedasticGaussianProcessRegressor`<br>**6)** `MultiTaskGaussianProcessRegressor`<br>**7)** `SpectralMixtureGaussianProcessRegressor`<br>**8)** `DeepKernelGaussianProcessRegressor` | Exact, sparse, classification (binary + OvR), heteroscedastic, multi-task ICM, spectral mixture, deep kernel, and kernel composition (`RBF`, Matérn, RQ, Periodic, Linear) | GP kernel classes + `predict_uq` | `notebooks/gp/GP_Model_Comparison.ipynb` (see all under `notebooks/gp/`) |
